...
group.add_argument("--chapter-duration", help="The length of a chater durection")
...
def get_chapters(file_path: str) -> List[Chapter]:
    ...
    chapters = []
    start_time = 0
    book_length = int(float(chapters_info['chapters'][-1]['end_time']))
    duration = 27
    chapters = [Chapter(start_time, start_time + duration, str(index), index + 1)
                    for index, start_time in enumerate(range(0, book_length+duration, duration))]
